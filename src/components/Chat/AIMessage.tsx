    ...
    <div className="animate-msg-in self-start max-w-full bg-white border border-gdf-border rounded-tl rounded-2xl overflow-hidden">
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-2.5 mb-4 pb-3 border-b border-[#EAF2ED]">
          ...
          <div>
            ...
            <div className="text-[11px] text-[#6B8B73] mt-0.5">Assistente oficial de serviços públicos</div>
          </div>
        </div>
        ...
        {data.blocks.map((b, i) => {
          ...
          return (
            <div key={i} className="bg-gdf-soft border border-gdf-border rounded-xl p-3.5 my-2.5">
              <h4 className="flex items-center gap-1.5 text-[10px] font-bold tracking-widest uppercase text-[#6B8B73] mb-2">
                {BIcon && <BIcon size={12} />}
                {b.title}
              </h4>
              {b.docs ? (
                <div className="flex flex-wrap gap-1.5">
                  {b.docs.map(d => (
                    <span key={d} className="inline-flex items-center gap-1 text-xs px-2 py-1 bg-white border border-gdf-border rounded-md text-[#3D5445]">
                      <Paperclip size={11} className="text-[#6B8B73]" /> {d}
                    </span>
                  ))}
                </div>
              ) : (
        ...
        {/* Steps */}
        <p className="text-[10px] font-bold tracking-widest uppercase text-[#6B8B73] mt-4 mb-2">Passo a passo</p>
        ...
        {data.contact && (
          <div className="bg-gdf-soft border border-gdf-border rounded-xl p-3.5 mt-3">
            <div className="text-[10px] font-bold tracking-widest uppercase text-[#6B8B73] mb-2.5">Onde ir presencialmente</div>
            <div className="flex flex-col gap-2">
              {[
                ...
                <div key={text} className="flex items-start gap-2 text-sm text-[#3D5445]">
        ...
        <div className="flex items-center gap-2 mt-5 pt-4 border-t border-[#EAF2ED]">
          ...
          <button
            ...
            className={`inline-flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-lg border transition-all ${
              feedback === 'pos' ? 'bg-verde-light border-verde text-verde' : 'bg-white border-gdf-border text-[#6B8B73] hover:border-verde hover:text-verde hover:bg-verde-light'
            }`}
          >
          ...
          <button
            ...
            className={`inline-flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-lg border transition-all ${
              feedback === 'neg' ? 'bg-red-50 border-red-400 text-red-600' : 'bg-white border-gdf-border text-[#6B8B73] hover:border-red-400 hover:text-red-600 hover:bg-red-50'
            }`}
          >
      ...
      {data.related && data.related.length > 0 && (
        <div className="bg-gdf-soft border-t border-gdf-border px-5 py-4">
          <div className="text-[10px] font-bold tracking-widest uppercase text-[#6B8B73] mb-2.5">Você também pode perguntar</div>
          <div className="flex flex-wrap gap-1.5">
            {data.related.map(r => (
              <button
                key={r}
                onClick={() => onRelated(r)}
                className="inline-flex items-center gap-1 text-xs font-medium text-[#3D5445] bg-white border border-gdf-border px-3 py-1.5 rounded-full hover:bg-verde-light hover:border-verde hover:text-verde transition-all"
              >
